...
import() { local file="$1" name id
    while read name id; do
        grep -q "^$name:" /etc/passwd || adduser --disabled-password --no-create-home --uid "$id" --gecos "" "$name"
...
user() { local name="$1" passwd="$2" id="${3:-""}" group="${4:-""}" \
    ...
    grep -q "^$name:" /etc/passwd ||
        adduser --disabled-password --no-create-home ${group:+--ingroup $group} ${id:+--uid $id} --gecos "" "$name"
